  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error: qErr } = await supabase
        .from("email_items")
        .select("id,from_email,subject,body_text,summary_json,status,is_relevant")
        .eq("id", params.id)
        .maybeSingle();
    ...
    <div className="space-y-8">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="text-sm text-black/60">Review the summary and draft. Sending always requires manual approval.</p>
        </div>
        ...
        <>
            ...
            <CardContent className="space-y-2">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <div className="mt-1 text-xs text-black/55">{item.from_email || "(unknown sender)"}</div>
                </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Generated by the LLM, validated against a JSON schema.</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Edit freely. You can also instruct the LLM to revise.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    onClick={() =>
                      reviseDraft(params.id, draftText, instruction, setBusy, setError, setDraftText, () =>
                        setInstruction("")
                      )
                    }
                  >
                ...
                {drafts.length ? (
                  <div className="text-xs text-black/50">
                    Latest draft version: <span className="font-medium text-black/70">{drafts[0].version}</span>
                  </div>
                ) : null}
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Plain text (HTML is converted to text on ingest).</CardDescription>
            </CardHeader>
            <CardContent>
              <pre className="whitespace-pre-wrap text-sm leading-relaxed text-black/70">{item.body_text || ""}</pre>
            </CardContent>
          </Card>
        </>
      ) : null}
...
function SummaryBlock({ summary }: { summary: unknown }) {
  ...
  const bullets = Array.isArray(s.summary_bullets) ? s.summary_bullets.map((x) => String(x)) : [];
  const wants = Array.isArray(s.what_they_want) ? s.what_they_want.map((x) => String(x)) : [];
  ...
  return (
    <div className="space-y-4 text-sm text-black/70">
      ...
      {wants.length ? (
        <div>
          <div className="text-xs font-medium text-black/55">What they want</div>
          <ul className="mt-2 list-disc space-y-1 pl-5">
            {wants.map((b, i) => (
              <li key={i}>{b}</li>
            ))}
      ...
      {s.suggested_next_step ? (
        <div>
          <div className="text-xs font-medium text-black/55">Suggested next step</div>
